...
export class EquipmentPage {
  ...
  private _loadEquipmentcats(): void {
    this.db.getEquipmentcats()
      .then((equipmentcats: Array<Equipmentcat>) => {
        ...
        this.selectedEquipments = equipmentcats;
      });
  }
          ...
          if (oReq.status >= 200 && oReq.status < 300) {
            ...
            json.forEach(item => {
              ...
              setTimeout(() => {
                ...
                this.newArray = [];
                this.newArray.push(equipmet);

                this.newArray.forEach(arrayEquipment => {
                  this.db.saveEquipmentcat(arrayEquipment).then(res => {
                    this._loadEquipmentcats();
                  });
                });
              }, 3000);
  ...
  pm(equipment) {
    ...
    this.db.getWorkorders().then(res => {
      ...
      //if (this.alert == false) {
        let modal = this.modalCtrl.create('AddfrequencyPage', { type: 'Equipment', equipment: equipment });
        modal.onDidDismiss((data) => {
          this._loadEquipmentcats();
        });
        modal.present();
  ...
  wo(equipment){
    ...
    this.db.getWorkorders().then(res => {
      ...
      //if (this.alert == false) {
        let modal = this.modalCtrl.create('AddorderPage', { type: 'Equipment', equipment: equipment });
        modal.onDidDismiss((data) => {
          this._loadEquipmentcats();
        });
        modal.present();
  ...
  checkboxClicked(equipment, event) {
    if (event._value) {
    ...
    this.newEquipments = this.selectedEquipments.filter(data => data['selected'] == true)
  ...
  deleteSelected() {
    if (this.showButton == true) {
       const alert = this.alertCtrl.create({
         title: 'Delete Equipment category',
         message: 'Are you sure you want to delete Selected Equipment category: ',
         buttons: [
           {
             text: 'Cancel',
             role: 'cancel'
           },
           {
             text: 'Confirm',
             handler: () => {
               this.newEquipments.forEach(equipmentcat => {
                 this.db.deleteEquipmentcat(equipmentcat)
                   .then((success: boolean) => {
                     this._loadEquipmentcats();
                     this.tableCheck = false;
    ...
    this.equipmentcats.forEach(equipments => {
      this.db.deleteEquipmentcat(equipments).then(res => {
        this._loadEquipmentcats();
      });
    })
  }
